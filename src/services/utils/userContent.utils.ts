...
export const getOpenAIRecommendation = async (watchedContentNames: string[]): Promise<string[]> => {
  let chatCompletionContent = "Based on these series and movies in array of strings format below, give me 10 series and movies recommendation with the titles in just one string splited by comma, don't write anything else, just the string, and don't send the same titles that are in the array of strings below: ";
